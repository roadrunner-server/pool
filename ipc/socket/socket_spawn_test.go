...
func Test_Unix_Start2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, log).SpawnWorkerWithContext(t.Context(), cmd)
	assert.NoError(t, err)
	assert.NotNil(t, w)
...
func Benchmark_Tcp_SpawnWorker_Stop2(b *testing.B) {
	...
	for b.Loop() {
		...
		w, err := f.SpawnWorkerWithContext(b.Context(), cmd)
		if err != nil {
			b.Fatal(err)
		}
		go func() {
...
func Benchmark_Tcp_Worker_ExecEcho2(b *testing.B) {
	...
	ctx, cancel := context.WithTimeout(b.Context(), time.Second*15)
	defer cancel()
